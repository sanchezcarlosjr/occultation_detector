...
def discretize(X):
    transformed_X = np.array(X).reshape(-1,1)
    est = KBinsDiscretizer(n_bins=len(X), encode='ordinal', strategy='uniform', subsample=None)
...
class Discretizer:
    ...
    def discretize_occultation_features(self, file):
        df = pd.read_csv(file)
        df = df.iloc[: , 1:]
        del df['D']
        del df['z']
        ...
        for column in df.columns:
            discretizer = self.load(f'../checkpoints/k_bins_discretizer_{column}.pkl')
            discretizers[column] = discretizer
            X = np.array(df[column]).reshape(-1,1)
            df[column] = discretizer.transform(X).reshape(-1)
        return df
    def dump(self):
        with open(f'../checkpoints/k_bins_discretizer_diameter.pkl', 'wb') as f:
            # diameter is in meters
            pickle.dump(discretize([0.5*1000,1*1000,2*1000,5*1000,10*1000]), f)
        with open(f'../checkpoints/k_bins_discretizer_distance.pkl', 'wb') as f:
            # distance is in astronomic units
            pickle.dump(discretize([30,40,50]), f)
        with open(f'../checkpoints/k_bins_discretizer_impact_parameter.pkl', 'wb') as f:
            pickle.dump(discretize([0,0.25,0.5,0.75]), f)
...
def log_results(occultation_features, *datasets):
    ...
    y_true = np.array([occultation_features['diameter'].to_numpy(),occultation_features['distance'].to_numpy(), occultation_features['impact_parameter'].to_numpy()]).T
